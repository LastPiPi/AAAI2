...
from meta_models import MetaNet



################################################################################
def main(args, meta_net):
    ...
    # meta net optimizer
    optimizer_meta = torch.optim.AdamW(meta_net.parameters(), lr=1e-5,
                                       weight_decay=0.0,  # args.wdecay, # meta should have wdecay or not??
                                       amsgrad=True, eps=1e-8)

    # data_loader
    train_dataset_meta = make_dataset(
        cfg['dataset_name'], True, cfg['train_split'], **cfg['dataset_meta']
    )
    train_loader_meta = make_data_loader(
        train_dataset_meta, True, rng_generator, **cfg['loader'])
...
if __name__ == '__main__':
    ...
    '''MLC'''
    # meta net
    expand_dim = 512  # y embedding
    hx_dim = 512  # fpn_feats
    h_dim = 128  # linear_hidden_layer
    meta_net = MetaNet(hx_dim, expand_dim, h_dim)
