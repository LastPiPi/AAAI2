...
class MetaAdamW(AdamW):
    ...
    def meta_step(self, grads):
        ...
        for (name, parameter), grad in zip(self.net.named_parameters(), grads):
            parameter.detach_()
            if weight_decay != 0:
                grad.add_(parameter, alpha=weight_decay)
            if 'step' not in self.state[parameter].keys():
                self.state[parameter]['step'] = 0

            exp_avg, exp_avg_sq = self.state[parameter].get('exp_avg', torch.zeros_like(parameter)), \
                                             self.state[parameter].get('exp_avg_sq', torch.zeros_like(parameter))


            exp_avg.mul_(betas[0]).add_(grad, alpha=1 - betas[0])
            exp_avg_sq.mul_(betas[1]).addcmul_(grad, grad, value=1 - betas[1])

            denom = (exp_avg_sq.sqrt() / math.sqrt(1 - betas[1] ** (self.state[parameter]['step'] + 1))).add_(eps)

            step_size = lr / (1 - betas[0] ** (self.state[parameter]['step'] + 1))

            self.set_parameter(self.net, name, parameter.addcdiv_(exp_avg, denom, value=-step_size))

            self.state[parameter]['exp_avg'] = exp_avg
            self.state[parameter]['exp_avg_sq'] = exp_avg_sq
