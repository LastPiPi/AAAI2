...
class PtTransformer(nn.Module):
    ...
    def forward(self, video_list, return_h=False):
        ...
        if self.training:
            ...
            if return_h:
                ...
                return pred_offsets, fpn_feats, fpn_masks, out_cls_logits, gt_cls_labels, gt_offsets
            else:
    ...
    def label_points_single_video(self, concat_points, gt_segment, gt_label):
        ...
        if self.train_center_sample == 'radius':
            ...
            # F T x N
            inside_gt_seg_mask = center_seg.min(-1)[0] > 0     #标记出s,e都大于0的部分
        else:
            # inside an gt action
            inside_gt_seg_mask = reg_targets.min(-1)[0] > 0
        # limit the regression range for each location
        '''inside_gt_seg_mask_full = reg_targets.min(-1)[0] > 0
        inside_gt_seg_mask_full = inside_gt_seg_mask_full.cpu().numpy()'''
        max_regress_distance = reg_targets.max(-1)[0]
        # F T x N
        inside_regress_range = torch.logical_and(
            (max_regress_distance >= concat_points[:, 1, None]),   #concat_points[:, 1, None],concat_points[:, 2, None]限制了最大回归范围
            (max_regress_distance <= concat_points[:, 2, None])
        )
        # if there are still more than one actions for one moment
        # pick the one with the shortest duration (easiest to regress)
        lens.masked_fill_(inside_gt_seg_mask==0, float('inf'))
        lens.masked_fill_(inside_regress_range==0, float('inf'))         #既满足在动作片段中心内，又满足在回归范围内的时刻才会被选到
        # F T x N -> F T
        ...
        #min_len_mask_array = min_len_mask.cpu().numpy()
